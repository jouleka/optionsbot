...
async def test_add_to_watchlist_persists_and_validates(
    ...
    with patch(
        ...
        result = await add(symbol="aapl", notes=None, ctx=FakeCtx(server_context))
...
async def test_add_to_watchlist_idempotent(
    ...
    with patch(
        ...
        resolver.stock = AsyncMock()
        ResolverCls.return_value = resolver
        result = await add(symbol="MSFT", notes=None, ctx=FakeCtx(server_context))
...
async def test_add_to_watchlist_rejects_unknown_symbol(
    ...
    with patch(
        ...
        ResolverCls.return_value = resolver
        result = await add(symbol="ZZZZ", notes=None, ctx=FakeCtx(server_context))
...
async def test_add_to_watchlist_rejects_empty_symbol(
    ...
    tools = get_tools(register)
    add = tools["add_to_watchlist"]
    result = await add(symbol="   ", notes=None, ctx=FakeCtx(server_context))
...
async def test_remove_from_watchlist_deletes_row(
    ...
    result = await remove(symbol="AAPL", ctx=FakeCtx(server_context))
...
async def test_remove_from_watchlist_unknown_symbol_returns_removed_false(
    ...
    tools = get_tools(register)
    remove = tools["remove_from_watchlist"]
    result = await remove(symbol="NOPE", ctx=FakeCtx(server_context))
...
async def test_remove_from_watchlist_preserves_snapshots(
    ...
    tools = get_tools(register)
    remove = tools["remove_from_watchlist"]
    await remove(symbol="AAPL", ctx=FakeCtx(server_context))
    with server_context.engine.connect() as conn:
...
async def test_list_watchlist_empty(server_context: ServerContext) -> None:
    ...
    result = await listfn(ctx=FakeCtx(server_context))
...
async def test_set_view_override_sets_direction(
    ...
    result = await setfn(
        symbol="AAPL", direction="bull", iv_regime=None, ctx=FakeCtx(server_context)
    )
    assert result["ok"] is True
...
async def test_set_view_override_clears_with_none(
    ...
    setfn = tools["set_view_override"]
    result = await setfn(
        symbol="AAPL", direction=None, iv_regime=None, ctx=FakeCtx(server_context)
    )
...
async def test_set_view_override_rejects_invalid_direction(
    ...
    setfn = tools["set_view_override"]
    result = await setfn(
        symbol="AAPL", direction="moonshot", iv_regime=None, ctx=FakeCtx(server_context)
    )
    assert result["ok"] is False
...
async def test_set_view_override_unknown_symbol(
    ...
    setfn = tools["set_view_override"]
    result = await setfn(
        symbol="NOPE", direction="bull", iv_regime=None, ctx=FakeCtx(server_context)
    )
    assert result["ok"] is False
    assert result["error"] == "unknown_symbol"
